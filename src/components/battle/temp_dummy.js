export const TEMP_DUMMY = [
  {
    battleId: 1,
    battleGenre: {
      genreValue: 'HIPHOP_RAP',
      genreName: '힙합/랩',
    },
    challenged: {
      postId: 1,
      music: {
        musicId: 'ABCD1234',
        singer: 'Gist',
        title: '신경 쓸 게 많아서 (Feat. The Quiett)',
        musicUrl:
          'https://audio-ssl.itunes.apple.com/itunes-assets/AudioPreview112/v4/31/18/8e/31188e1f-0079-2d1f-7428-29daa4163c07/mzaf_4295413803000543523.plus.aac.p.m4a',
        albumCoverUrl:
          'https://is5-ssl.mzstatic.com/image/thumb/Music122/v4/ca/20/55/ca205584-83ae-8be5-a415-03df7baae6cb/8809658318466_Cover.jpg/100x100bb.jpg',
        genre: {
          genreValue: 'HIPHOP_RAP',
          genreName: '힙합/랩',
        },
      },
    },
    challenging: {
      postId: 2,
      music: {
        musicId: 'ABCD1235',
        singer: 'THAMA, Jayci yucca (제이씨 유카)',
        title: '떠나 (Prod. PATEKO (파테코))',
        musicUrl:
          'https://audio-ssl.itunes.apple.com/itunes-assets/AudioPreview112/v4/31/18/8e/31188e1f-0079-2d1f-7428-29daa4163c07/mzaf_4295413803000543523.plus.aac.p.m4a',
        albumCoverUrl:
          'https://is3-ssl.mzstatic.com/image/thumb/Music112/v4/52/6e/b5/526eb565-8444-3ec2-0392-c3ed55feb0b9/cover_KM0015957_1.jpg/100x100bb.jpg',
        genre: {
          genreValue: 'HIPHOP_RAP',
          genreName: '힙합/랩',
        },
      },
    },
  },
  {
    battleId: 2,
    battleGenre: {
      genreValue: 'ROCK_METAL',
      genreName: '락/메탈',
    },
    challenged: {
      postId: 1,
      music: {
        musicId: '1234435624',
        singer: '쏜애플',
        title: '한낮',
        musicUrl:
          'https://audio-ssl.itunes.apple.com/itunes-assets/AudioPreview125/v4/d0/5e/79/d05e79ae-d15a-d321-c304-ca726080a493/mzaf_14802460072071206033.plus.aac.p.m4a',
        albumCoverUrl:
          'https://is3-ssl.mzstatic.com/image/thumb/Music118/v4/40/62/09/40620991-3a47-b615-472a-18daca79eb57/capital_disease.jpg/100x100bb.jpg',
        genre: {
          genreValue: 'ROCK_METAL',
          genreName: '락/메탈',
        },
      },
    },
    challenging: {
      postId: 2,
      music: {
        musicId: '46564353',
        singer: '아가미',
        title: '쏜애플',
        musicUrl:
          'https://audio-ssl.itunes.apple.com/itunes-assets/AudioPreview115/v4/9e/0d/97/9e0d972b-0405-31c5-1e2d-42d8507c3ea6/mzaf_13637269421332642120.plus.aac.p.m4a',
        albumCoverUrl:
          'https://is3-ssl.mzstatic.com/image/thumb/Music124/v4/af/66/ba/af66ba60-8f7d-1e51-6e08-1588b7f28f81/I_Often_Stammer_and_Forget_the_Way_to_Sleep.jpg/100x100bb.jpg',
        genre: {
          genreValue: 'ROCK_METAL',
          genreName: '락/메탈',
        },
      },
    },
  },
];
